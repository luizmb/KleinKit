...
public extension UIViewController {
    ...
    /**
     * You must call this method from viewDidLayoutSubviews in your content view controlers so it fixes size
     * and position of the side menu when the screen rotates.
     * A convenient way to do it might be creating a subclass of UIViewController that does precisely that and
     * then subclassing your view controllers from it.
     */
...
open class ENSideMenu: NSObject, UIGestureRecognizerDelegate {
    ...
    public init(sourceView: UIView, menuPosition: ENSideMenuPosition, blurStyle: UIBlurEffectStyle = .light) {
        ...
        outterView.backgroundColor = .clear
    ...
    func updateFrame() {
        ...
        let menuFrame = CGRect(
            x: (menuPosition == .left) ? isMenuOpen ? 0 : -menuWidth - 1.0 : isMenuOpen ? width - menuWidth : width + 1.0,
    ...
    fileprivate func toggleMenu (_ shouldOpen: Bool) {
        outterView.sendSubview(toBack: self.sideMenuContainerView)
    ...
    @objc internal func handlePan(_ recognizer: UIPanGestureRecognizer) {
        ...
        default:
            ...
            let shouldClose = menuPosition == .left
                ? !leftToRight && sideMenuContainerView.frame.maxX < menuWidth
                : leftToRight && sideMenuContainerView.frame.minX > (sourceView.frame.size.width - menuWidth)
